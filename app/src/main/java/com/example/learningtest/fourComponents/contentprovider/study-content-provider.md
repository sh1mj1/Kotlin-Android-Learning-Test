...
`URI` 와 `ContentResolver` 인터페이스로 데이터를 주고 받는다.
...
1. **비동기 로딩**: `ContentProvider`로부터 데이터를 가져오는 작업을 백그라운드 스레드에서 수행한다. 이를 통해 UI 스레드가 차단되는 것을 방지하여 앱의
   응답성을 유지한다.
2. **생명주기 관리**: `Activity` 또는 `Fragment`의 생명주기와 통합되어, 컴포넌트가 활성 상태일 때만 데이터를 로드하고, 비활성 상태가 되면 자동으로 로더를
   중지하거나 리소스를 해제한다.
3. **자동 업데이트**: `ContentProvider`의 데이터가 변경되면(`ContentObserver`를 통해 알림 수신), `CursorLoader`는 자동으로 새로운
   데이터를 다시 로드하고 UI에 반영한다. 개발자가 직접 데이터 변경을 감지하고 UI를 갱신하는 코드를 작성할 필요가 없다.
4. **데이터 유지**: 화면 회전과 같은 구성 변경 시에도 로드된 데이터를 유지하여 불필요한 데이터 재로드를 방지한다.

## Contracts 에 접근하는 ContentProvider 예시

Contacts 에 접근하는 예시는
앱이 다른 앱(여기서는 Android 시스템의 연락처 앱)의 `ContentProvider`에 접근하여 데이터를 가져오는 경우이다.
이때, 앱은 연락처 데이터의 URI와 필요한 권한을 알고 있어야 한다.
데이터 제공자(연락처 앱)는 `ContentProvider`를 통해 데이터를 외부에 공개하고, 접근 권한을 설정한다.
이를 통해 앱은 직접 데이터베이스에 접근하지 않고도 안전하게 데이터를 공유받을 수 있다.
...
앱 내부의 다른 모듈과 데이터를 주고받는 경우이다.

이 경우:

1. 앱 개발자는 `ContentProvider`를 직접 만들고,
2. 데이터에 접근할 수 있는 URI를 정의하며,
3. 필요한 경우 접근 권한도 설정한다.

이를 통해 앱은 자신의 데이터를 체계적으로 관리하고, 필요에 따라 다른 구성 요소와 안전하게 공유할 수 있다.
...
> 문서, 파일 또는 바이트 스트림의 성격과 형식을 나타내는 표준화된방식입니다.
> 쉽게 말해, "이 데이터는 어떤 종류의 데이터인가?"를 알려주는 식별자라고 생각할 수 있다.
> 원래는 이메일 시스템에서 다양한 형식의 파일을 첨부하기 위해 개발되었다.
> 하지만 현재는 인터넷 전반, 특히 웹(HTTP)에서 널리 사용된다.
> `type/subtype` 으로 구분된다. ex: `text/plain`, `image/jpeg`, `application/json`
